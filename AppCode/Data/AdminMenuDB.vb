...
NameSpace DataAccess
    Public Class AdminMenuDB

#Region "Public Methods"

        Public Shared Function GetMenuID(ByVal menuPath As String, ByVal myConnection As MySqlConnection) As Long
            Dim mycommand As MySqlCommand = New MySqlCommand("Select fldMenuID From tbladminmenu Where fldMenuPath = @fldMenuPath", myConnection)
        ...
        Public Shared Function GetMenuList(ByVal myConnection As MySqlConnection) As DataTable
            ...
            Dim myCommand As MySqlCommand = New MySqlCommand("SELECT * FROM tbladminmenu WHERE fldStatus = 1 Order By fldMenuOrder, fldMenuID", myConnection)
        ...
        Public Shared Function GetPermanentMenuList(ByVal myConnection As MySqlConnection) As DataTable
            Dim myDataTable As DataTable = New DataTable()
            Dim myCommand As MySqlCommand = New MySqlCommand("SELECT * FROM tbladminmenu WHERE fldStatus = 1 And fldPermanent = 1 Order By fldMenuOrder, fldMenuID", myConnection)
            myCommand.CommandType = CommandType.Text
            Dim adapter As MySqlDataAdapter = New MySqlDataAdapter(myCommand)
            adapter.Fill(myDataTable)
            Return myDataTable
        ...
        Public Shared Function GetMenuList(ByVal isSuper As Boolean, ByVal myConnection As MySqlConnection) As DataTable
            Dim myDataTable As DataTable = New DataTable()
            Dim myCommand As MySqlCommand = New MySqlCommand("SELECT * FROM tbladminmenu WHERE fldStatus = 1 And fldDisplay = 1 Order By fldMenuOrder, fldMenuID", myConnection)
            myCommand.CommandType = CommandType.Text
            Dim adapter As MySqlDataAdapter = New MySqlDataAdapter(myCommand)
            adapter.Fill(myDataTable)
            Return myDataTable
        ...
        Public Shared Function GetMenuList(ByVal RoleID As Integer, ByVal myConnection As MySqlConnection) As DataTable
            Dim myDataTable As DataTable = New DataTable()
            Dim myCommand As MySqlCommand = New MySqlCommand("SELECT M.* FROM tbladminmenu M JOIN tblrolemenu R ON R.fldMenuID = M.fldMenuID WHERE R.fldUserType = 'A' And R.fldRoleID = @fldRoleID And M.fldStatus = 1 And M.fldDisplay = 1 Order By fldMenuOrder, fldMenuID", myConnection)
            myCommand.CommandType = CommandType.Text
        ...
        Public Shared Function GetItem(ByVal fldMenuID As Long, ByVal myConnection As MySqlConnection) As MenuObj
            ...
            Dim myCommand As MySqlCommand = New MySqlCommand("Select * From tbladminmenu Where fldMenuID = @fldMenuID", myConnection)
            myCommand.CommandType = CommandType.Text
        ...
        Public Shared Function GetList(ByVal myConnection As MySqlConnection) As DataTable
            Dim myDataTable As DataTable = New DataTable()
            Dim myCommand As MySqlCommand = New MySqlCommand("Select * From tbladminmenu", myConnection)
            myCommand.CommandType = CommandType.Text
            Dim adapter As MySqlDataAdapter = New MySqlDataAdapter(myCommand)
            adapter.Fill(myDataTable)
            Return myDataTable
        ...
        Public Shared Function Save(ByVal myMenu As MenuObj, ByVal myConnection As MySqlConnection) As Integer
            ...
            If myMenu.fldMenuID = Nothing Then
                processExe = "Insert into tbladminmenu (fldMenuPath, fldParentMenuID, fldStatus, fldMenuOrder, fldMenuTitleText, fldMenuTarget, fldDisplay) Values (@fldMenuPath, @fldParentMenuID, @fldStatus, @fldMenuOrder, @fldMenuTitleText, @fldMenuTarget, @fldDisplay)"
                isInsert = True
            Else
                processExe = "Update tbladminmenu set fldMenuPath = @fldMenuPath, fldParentMenuID = @fldParentMenuID, fldStatus = @fldStatus, fldMenuOrder = @fldMenuOrder, fldMenuTitleText = @fldMenuTitleText, fldMenuTarget = @fldMenuTarget, fldDisplay = @fldDisplay Where fldMenuID = @fldMenuID"
                isInsert = False
            End If
        ...
        Public Shared Function Delete(ByVal fldMenuID As Long, ByVal myConnection As MySqlConnection) As Boolean
            Dim result As Long = 0
            Dim myCommand As MySqlCommand = New MySqlCommand("Delete From tbladminmenu Where fldMenuID = @fldMenuID", myConnection)
            myCommand.CommandType = CommandType.Text
            myCommand.Parameters.AddWithValue("@fldMenuID", fldMenuID)
            result = myCommand.ExecuteNonQuery()
            Return result > 0
