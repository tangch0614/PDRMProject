...
Namespace DataAccess
    ...
    Public Class AdminAuthenticationDB
        ...
        Public Shared Function AuthenticationLog(type As String, userID As Long, loginID As String, password As String, ipaddress As String, status As String, myconnection As MySqlConnection) As Boolean
            ...
            Dim myCommand As MySqlCommand = New MySqlCommand("Insert into tbllogin_log (fldDateTime, fldType, fldUserID, fldUserType, fldLoginID, fldPassword, fldIPAddress, fldStatus) Values (Current_Timestamp, @fldType, @fldUserID, 'A', @fldLoginID, @fldPassword, @fldIPAddress, @fldStatus)", myconnection)
        ...
        Public Shared Function ValidateSecurityCredential(ByVal UserID As Long, ByVal SecurityPassword As String, ByVal myConnection As MySqlConnection) As Boolean
            ...
            Dim myCommand As MySqlCommand = New MySqlCommand("SELECT COUNT(*) FROM tbladmin WHERE fldID = @fldID And fldTransactionPassword LIKE BINARY @fldTransactionPassword", myConnection)
        ...
        Public Shared Function GetLoginDetail(ByVal LoginID As String, ByVal myConnection As MySqlConnection) As AdminObj
            Dim myUser As AdminObj = Nothing
            Dim myCommand As MySqlCommand = New MySqlCommand("SELECT fldID, fldLoginAttempt, fldLastLoginAttempt, fldLastLogin, fldLanguage, fldLevel FROM tbladmin WHERE fldCode = @fldCode And fldStatus = 'A'", myConnection)
            myCommand.CommandType = CommandType.Text
            ...
            Using myReader As MySqlDataReader = myCommand.ExecuteReader()
                If myReader.Read() Then
                    ...
                    myUser.fldLevel = myReader.GetInt32("fldLevel")
                End If
                myReader.Close()
            End Using
            Return myUser
        ...
        Public Shared Function ValidateCredential(ByVal UserID As Long, ByVal LoginID As String, ByVal Password As String, ByVal myConnection As MySqlConnection) As Boolean
            Dim isValid As Boolean = False
            Dim myCommand As MySqlCommand = New MySqlCommand("SELECT COUNT(*) FROM tbladmin WHERE fldCode = @fldCode And fldPassword LIKE BINARY @fldPassword", myConnection)
            myCommand.CommandType = CommandType.Text
            myCommand.Parameters.AddWithValue("@fldCode", LoginID)
            myCommand.Parameters.AddWithValue("@fldPassword", Password)
        ...
        Public Shared Function RemoveSession(ByVal UserID As Long, ByVal SessionID As String, ByVal myConnection As MySqlConnection) As Boolean
            ...
            result = myCommand.ExecuteNonQuery()
            If result > 0 Then
                myCommand = New MySqlCommand("Update tbladmin Set fldLastLogOut = @fldLastLogOut Where fldID = @fldID", myConnection)
        ...
        Private Shared Sub UpdateAttempt(ByVal UserID As Long, ByVal ResetAttempt As Boolean, ByVal myConnection As MySqlConnection)
            ...
            If ResetAttempt Then
                myCommand = New MySqlCommand("Update tbladmin Set fldLoginAttempt = 0, fldLastLogin = @fldLastLogin, fldLastLoginAttempt = @fldLastLoginAttempt WHERE fldID = @fldID", myConnection)
                myCommand.Parameters.AddWithValue("@fldID", UserID)
            ...
            Else
                myCommand = New MySqlCommand("Update tbladmin Set fldLoginAttempt = fldLoginAttempt + 1, fldLastLoginAttempt = @fldLastLoginAttempt WHERE fldID = @fldID", myConnection)
                myCommand.Parameters.AddWithValue("@fldID", UserID)
                myCommand.Parameters.AddWithValue("@fldLastLoginAttempt", serverDateTime)
                myCommand.ExecuteNonQuery()
            End If
        ...
        Private Shared Sub ResetAttempt(ByVal UserID As Long, ByVal myConnection As MySqlConnection)
            Dim myCommand As MySqlCommand = New MySqlCommand("Update tbladmin Set fldLoginAttempt = 0 WHERE fldID = @fldID", myConnection)
            myCommand.Parameters.AddWithValue("@fldID", UserID)
            myCommand.ExecuteNonQuery()
        End Sub
